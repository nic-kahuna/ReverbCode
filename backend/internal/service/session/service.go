...
// SendAdmitted atomically checks native project admission before a background message.
func (s *Service) SendAdmitted(ctx context.Context, id domain.SessionID, message string) error {
	return toAPIError(s.manager.SendAdmitted(ctx, id, message))
}

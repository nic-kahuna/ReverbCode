...
func TestProjectSetConfig_DesktopProjectsAdmissionJSON(t *testing.T) {
	cfg := setConfigEnv(t)
	srv, capture := projectServer(t, http.StatusOK, `{"project":{"id":"demo","path":"/repo/demo"}}`)
	writeRunFileFor(t, cfg, srv)
	...
	_, errOut, err := executeCLI(t, Deps{
		ProcessAlive: func(int) bool { return true },
	}, "project", "set-config", "demo", "--config-json", `{"desktopProjectsAdmission":true}`)
	if err != nil {
		t.Fatalf("unexpected error: %v\nstderr=%s", err, errOut)
	}
	var got setConfigRequest
	...
	if got.Config.DesktopProjectsAdmission == nil || !*got.Config.DesktopProjectsAdmission {
		t.Fatalf("desktop-projects admission request = %#v", got.Config.DesktopProjectsAdmission)
	}
}

...
func TestCompatibilityCommandIsReadOnlyAndTyped(t *testing.T) {
	...
	if status.Schema != "ao-compatibility/v1" || status.State != "missing_legacy" || !status.InspectionOnly || !status.StartPausedSupported || status.SupportedProtocol != 1 {
		t.Fatalf("wire %s", out)
	}
	...
	future := []byte("{\"schema\":\"ao-data-compatibility/v1\",\"requiredProtocol\":2}\n")
	...
	if status.State != "unsupported" || status.RequiredProtocol != 2 {
